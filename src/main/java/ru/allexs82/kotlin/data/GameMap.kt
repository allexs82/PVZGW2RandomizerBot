...
sealed class GameMap(open val name: String, open val modes: List<Modes>) {

    data class CoopGameMap(
        override val name: String,
        val additionalModes: List<Modes>
    ) : GameMap(name, listOf(Modes.TEAM_VANQUISH, Modes.VANQUISH_CONFIRMED, Modes.GNOME_BOMB, Modes.SUBURBINATION) + additionalModes)

    data class TTFGameMap(
        override val name: String,
        val mode: Modes
    ) : GameMap(name, listOf(Modes.TURF_TAKEOVER, mode))
    ...
    companion object {
        ...
        fun getRandomMapForModes(selectedModes: List<Modes>, mapsToExclude: List<GameMap>?): GameMap? {
            ...
            val filteredModes = selectedModes.toMutableList()
